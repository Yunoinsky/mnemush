...
Step "cargo build --release" {
    cargo build --manifest-path crates/mneme/Cargo.toml --release
}
...
Step "cargo clippy --lib --bins -D clippy::all" {
    cargo clippy --manifest-path crates/mneme/Cargo.toml --lib --bins -- -D clippy::all
}

# 4. fmt check
Step "cargo fmt --check" {
    cargo fmt --manifest-path crates/mneme/Cargo.toml --check
}
...
Step "mneme-client 测试" {
    Set-Location packages/mneme-client
    npm test
    Set-Location $repoRoot
}

# 7. MCP smoke (spawn mneme-mcp.exe, 覆盖 12+ 工具)
Step "python scripts/test-mcp.py" {
    python scripts/test-mcp.py
}

# 8. OpenCode 插件集成 (需要刚 build 的 mneme-mcp.exe)
Step "mneme-opencode 集成测试" {
    Set-Location packages/mneme-opencode
    npm test
    Set-Location $repoRoot
}

# 9. mneme 二进制 CLI 冒烟 + sync 端到端 (Git 作为传输层)
Step "mneme CLI + sync 端到端" {
    $bin = Join-Path $repoRoot "crates\mneme\target\release\mneme.exe"
    if (-not (Test-Path $bin)) { throw "未找到 $bin" }
    & $bin --version
    if ($LASTEXITCODE -ne 0) { throw "mneme --version 失败" }

    # 用一个临时数据目录, 避免碰真实 ~/.mneme
    $tmp = Join-Path $env:TEMP ("mneme-verify-" + [guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $tmp -Force | Out-Null
    $env:MNEME_DATA_DIR = $tmp
    try {
        & $bin add "hello" "windows verify" -c note
        if ($LASTEXITCODE -ne 0) { throw "mneme add 失败" }
        & $bin search "hello"
        if ($LASTEXITCODE -ne 0) { throw "mneme search 失败" }

        # sync init + import round-trip (验证 git 在 Windows 上工作)
        $syncDir = Join-Path $tmp "sync"
        & $bin sync init -d $syncDir
        if ($LASTEXITCODE -ne 0) { throw "mneme sync init 失败" }
        & $bin sync export -d $syncDir
        if ($LASTEXITCODE -ne 0) { throw "mneme sync export 失败" }
        ...
        Remove-Item Env:MNEME_DATA_DIR -ErrorAction SilentlyContinue
...
Write-Host "`n=========================================="
if ($failCount -eq 0) {
    Write-Host "✅ 全部通过! mneme 在 Windows 上验证成功。" -ForegroundColor Green
    exit 0
} else {
