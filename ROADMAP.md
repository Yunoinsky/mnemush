...
- API stability, comprehensive integration tests, docs site. **DONE** (partial — see ⚠️ below) — `#![warn(missing_docs)]` enforces /// on public items; `cargo doc` builds clean (5 pre-existing HTML lint warnings, no broken links). Three docs levels: Rust API → [docs.rs/mneme](https://docs.rs/mneme) (auto-publish on crates.io release); TS API → typedoc via `npm run docs:ts`; conceptual → GitHub-rendered markdown. Integration tests: 96 lib + 18 bin Rust + 36 client + 31 OpenCode + 26 hook = 207 tests.
