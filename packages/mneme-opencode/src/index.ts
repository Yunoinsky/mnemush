...
const plugin: OpenCodePlugin = ({ client: oc }) => {
  ...
  oc.on("tool.execute.after", async (event) => {
    ...
    // Skip mneme tools (any surface) — don't record our own failures
    // as "tool failure" memories. isMnemeTool() covers all 16 tools
    // (the old check only skipped two).
    if (!e || !e.name || isMnemeTool(e.name)) return;
  ...
  // name, latency, and errors to ~/.mneme/eval/<session>.ndjson,
  // keeping `mneme eval stats` covering both surfaces. result_count
        ...
        .then((list) => {
          if (list.length > 0) {
            const preview = list
              ...
              .join("\n");
            console.log(
              `[mneme] session.created: ${list.length} pending identity proposal(s):\n${preview}`,
            );
          }
        })
        .catch(() => {});
    } catch (e) {
      console.error(`[mneme] session.created failed: ${e}`);
    }
  });
  ...
  oc.on("session.deleted", async () => {
    ...
    try {
      ...
      // edge-decay, process-needs-review. Default ON, MNEME_*_ON_SESSION_END=off
      // can disable. Hard-delete (--isolate) is NEVER auto-run.
      await new Promise<void>((resolve) => {
        const proc = spawnChild("mneme", ["prune", "--apply", "--limit", limit], {
      ...
      await new Promise<void>((resolve) => {
        const proc = spawnChild("mneme", ["edge-decay"], {
          stdio: ["ignore", "ignore", "ignore"],
        });
        proc.on("exit", () => resolve());
        proc.on("error", () => resolve());
      ...
      await new Promise<void>((resolve) => {
        const proc = spawnChild("mneme", ["process-needs-review"], {
          stdio: ["ignore", "ignore", "ignore"],
        });
        proc.on("exit", () => resolve());
        proc.on("error", () => resolve());
      ...
      // MNEME_EVAL_PRUNE_ON_SESSION_END=off to opt out.
      if ((process.env.MNEME_EVAL_PRUNE_ON_SESSION_END ?? "on") !== "off") {
        await new Promise<void>((resolve) => {
          const proc = spawnChild("mneme", ["eval", "prune", "--apply"], {
            stdio: ["ignore", "ignore", "ignore"],
          });
          proc.on("exit", () => resolve());
          proc.on("error", () => resolve());
      ...
      console.error(`[mneme] session.deleted failed: ${e}`);
    }
  });
  ...
  registerTool({
    name: "mneme-memory-save-search-result",
    description:
      "Explicitly save one or more search hits as memories. Pass the " +
      "memory ids returned by a prior mneme-memory-search call; each " +
  ...
  registerTool({
    ...
    execute: async () => {
      await ensureConnected();
      const c = getClient();
      return tryRun("mneme_status failed", async () => {
  ...
  registerTool({
    name: "mneme-memory-action-create",
    description:
  ...
  registerTool({
    name: "mneme-memory-action-update",
    description:
