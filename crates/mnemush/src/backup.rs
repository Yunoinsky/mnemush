...
//! Backup and restore of the entire `~/.mnemush/` data directory.
//!
//! Format: gzipped tar archive. The first entry is `MANIFEST.json`
//! recording the mnemush version, schema_version, and live counts at
//! backup time. Remaining entries are the live data files:
//!
//!   - `mnemush.db` — captured via SQLite's online backup API (handles
//!     WAL/SHM atomically)
...
pub fn create_backup_to(
    ...
    if !db_path.exists() {
        return Err(MnemushError::Other(format!(
            "no database at {} — is mnemush initialized?",
            db_path.display()
        )));
    }
    ...
    let result = (|| -> Result<BackupMeta> {
        ...
        tar.into_inner()
            .map_err(|e| MnemushError::Other(format!("finalize tar: {}", e)))?
            .finish()
            .map_err(|e| MnemushError::Other(format!("finalize gzip: {}", e)))?;
        Ok(meta)
    })();
...
fn read_manifest(archive: &Path) -> Result<BackupMeta> {
    ...
    Err(MnemushError::Other(format!(
        "archive is missing MANIFEST.json (not a mnemush backup?): {}",
        archive.display()
    )))
}
...
mod tests {
    ...
    fn setup_home() -> tempfile::TempDir {
        ...
        // Create a minimal mnemush home: open the store (which runs the
    ...
    fn backup_then_restore_round_trip() {
        ...
        assert_eq!(meta_before.mnemush_version, meta_after.mnemush_version);
