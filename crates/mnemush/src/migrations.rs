        ...
        for sql in alters {
            ...
            if !has_column(tx, col)? {
                tx.execute_batch(sql)
                    .map_err(|e| MnemushError::Other(format!("migration v1->v2: {}", e)))?;
            }
        }
        tx.execute_batch(
            "CREATE INDEX IF NOT EXISTS idx_memory_active ON memory(deleted_at) WHERE deleted_at IS NULL;",
        )
        .map_err(|e| MnemushError::Other(format!("migration v1->v2 index: {}", e)))?;
        Ok(())
    }
}
...
impl Migration for V2ToV3 {
    ...
    fn up(&self, tx: &Transaction) -> Result<()> {
        ...
        for sql in alters {
            ...
            if !has_column(tx, col)? {
                tx.execute_batch(sql)
                    .map_err(|e| MnemushError::Other(format!("migration v2->v3: {}", e)))?;
            }
        }
        Ok(())
    }
...
impl Migration for V3ToV4 {
    ...
    fn up(&self, tx: &Transaction) -> Result<()> {
        tx.execute_batch(crate::embeddings::V3_TO_V4_SQL)
            .map_err(|e| MnemushError::Other(format!("migration v3->v4: {}", e)))?;
        Ok(())
    }
}
