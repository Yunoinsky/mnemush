//! Error types for mnemush.

use thiserror::Error;

#[derive(Debug, Error)]
pub enum MnemushError {
...
pub type Result<T> = std::result::Result<T, MnemushError>;

/// Convert a [`MnemushError`] (e.g. from strict `parse_*` calls) into a
...
impl From<MnemushError> for rusqlite::Error {
    fn from(e: MnemushError) -> Self {
        // Use ToSqlConversionFailure (not FromSqlConversionFailure)
        // so the Display impl shows the actual MnemushError message,
