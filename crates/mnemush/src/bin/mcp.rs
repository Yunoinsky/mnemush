...
use mnemush::edge::EdgeApi;
use mnemush::memory::MemoryApi;
use mnemush::schema::{Category, EdgeType, MemoryType, NewMemory, SearchOpts, Source, Tier};
    ...
    match req.method.as_str() {
        "initialize" => JsonRpcResponse::ok(
            ...
            json!({
                ...
                "serverInfo": { "name": "mnemush", "version": mnemush::VERSION },
                "capabilities": { "tools": {} }
            }),
        ),
        ...
        "tools/call" => {
            ...
            match tool_name {
                ...
                Some("mnemush_status") => call_mnemush_status(args, store, config, id),
...
fn call_mnemush_status(_p: Value, store: &Store, config: &Config, id: Value) -> JsonRpcResponse {
...
fn call_memory_save_search_result(
    ...
    let api = MemoryApi::new(store, config);
    let mut saved: Vec<mnemush::schema::Memory> = Vec::new();
...
fn call_memory_frontier(p: Value, _store: &Store, _config: &Config, id: Value) -> JsonRpcResponse {
    ...
    match api.memory_frontier() {
        Ok(v) => {
            let trimmed: Vec<&mnemush::schema::Memory> = v.iter().take(limit).collect();
            json_ok(id, &trimmed)
        }
        Err(e) => json_err(id, e.to_string()),
    }
...
fn call_memory_action_create(
    ...
    use mnemush::memory::MemoryApi;
    use mnemush::schema::{Category, MemoryType, NewMemory, Source, Tier};
    let title = match p.get("title").and_then(|v| v.as_str()) {
        Some(s) => s,
        None => return JsonRpcResponse::err(id, -32602, "missing title"),
    };
...
fn call_memory_action_update(
    ...
    use mnemush::memory::MemoryApi;
    use mnemush::schema::ActionStatus;
    let mid = match p.get("id").and_then(|v| v.as_str()) {
        Some(s) => s,
        None => return JsonRpcResponse::err(id, -32602, "missing id"),
    };
...
fn call_identity_propose(p: Value, id: Value) -> JsonRpcResponse {
    ...
    let evidence = p
        ...
        .and_then(|v| v.as_u64())
        .unwrap_or(1) as u32;
    match mnemush::identity::propose(target, content, reason, evidence) {
        Ok(p) => json_ok(id, &p),
        Err(e) => json_err(id, e.to_string()),
    }
}
...
fn call_identity_list_pending(p: Value, id: Value) -> JsonRpcResponse {
    ...
    let status = if let Some(s) = p.get("status").and_then(|v| v.as_str()) {
        match s {
            "pending" => Some(mnemush::identity::ProposalStatus::Pending),
            "approved" => Some(mnemush::identity::ProposalStatus::Approved),
            "rejected" => Some(mnemush::identity::ProposalStatus::Rejected),
            other => {
    ...
    match mnemush::identity::list_pending(status) {
        Ok(v) => json_ok(id, &v),
        Err(e) => json_err(id, e.to_string()),
    }
}
...
fn call_identity_approve(p: Value, id: Value) -> JsonRpcResponse {
    ...
    match mnemush::identity::approve(mid) {
        Ok(Some(v)) => json_ok(id, &v),
        Ok(None) => match mnemush::identity::find_proposal(mid) {
...
fn call_identity_reject(p: Value, id: Value) -> JsonRpcResponse {
    ...
    match mnemush::identity::reject(mid) {
        Ok(Some(v)) => json_ok(id, &v),
        Ok(None) => match mnemush::identity::find_proposal(mid) {
            Ok(Some(p)) => JsonRpcResponse::err(
                id,
...
mod tests {
    ...
    /// a fixed `~/.mnemush/identity/` by default). Each test acquires
    /// the lock, points MNEMUSH_DATA_DIR at a unique tempdir, runs, and
    ...
    /// Set MNEMUSH_DATA_DIR to a fresh tempdir, run the closure, restore.
    fn with_isolated_identity_dir<F: FnOnce()>(f: F) {
        let _guard = ID_LOCK.lock().unwrap();
        let tmp = tempfile::tempdir().unwrap();
        let prev = env::var("MNEMUSH_DATA_DIR").ok();
        env::set_var("MNEMUSH_DATA_DIR", tmp.path());
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(f));
        match prev {
            Some(v) => env::set_var("MNEMUSH_DATA_DIR", v),
            None => env::remove_var("MNEMUSH_DATA_DIR"),
        }
        // Re-evaluate default if needed (otherwise propsed target path
        // would be stale). We don't actually need this — propose_in
        // reads MNEMUSH_DATA_DIR at call time via default_identity_dir.
        if let Err(e) = result {
...
mod unknown_value_tests {
    ...
    fn memory_link_with_unknown_target_id_returns_not_found() {
        ...
        let src = mnemush::memory::MemoryApi::new(&store, &cfg)
            .add(mnemush::schema::NewMemory {
                content: "x".into(),
                title: "x".into(),
                category: mnemush::schema::Category::Note,
                memory_type: mnemush::schema::MemoryType::Semantic,
                tier: mnemush::schema::Tier::Global,
                context: None,
                tags: vec![],
                project: None,
                source: mnemush::schema::Source::Manual,
                importance: 0.5,
                override_half_life: None,
                never_prune: false,
                never_decay: false,
