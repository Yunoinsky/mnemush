...
impl<'a> MemoryApi<'a> {
    ...
    /// - `Some("default")` if MNEMUSH_PROJECT=default and cross-project
    ...
    pub fn add(&self, m: NewMemory) -> Result<AddResult> {
        if let Some(threat) = crate::scanner::scan(&m.content) {
            return Err(MnemushError::ScanBlocked(threat.to_string()));
        }
    ...
    /// (cross-project escape hatch, ignoring MNEMUSH_PROJECT).
    ...
    pub fn search(&self, query: &str, opts: SearchOpts) -> Result<Vec<SearchHit>> {
        ...
        // ── Semantic recall + blend (v1.0, opt-in) ────────────────
        // Vector search over the full corpus joins the FTS candidate
        // set, so queries with zero FTS overlap (e.g. a Chinese query
        // against English memories) can still surface relevant hits.
        // Then the merged set is re-ranked by blending normalized BM25
        // with cosine similarity. Falls back to BM25-only when
        // embeddings are disabled or the model can't be loaded.
        if self.config.embedding.enabled {
            ...
            if let Ok(emb) = crate::embeddings::cached_embedder(&model_id) {
                let qvec: Vec<f32> = emb
                    ...
                    .unwrap_or_default();
                if !qvec.is_empty() {
                    // Full-corpus cosine: semantic recall + blend input.
                    let all = self.store.all_embeddings(&model_id).unwrap_or_default();
                    ...
                    for (mid, _cos) in scored.into_iter().take((limit * 2).max(5) as usize) {
                        ...
                        if let Ok(Some(mem)) = self.get(&mid) {
                            hits.push(SearchHit {
                                memory: mem,
                                score: 0.0,
                                bm25: 0.0, // vector-only: no BM25 score
                                retrievability: 1.0,
                            });
                        }
                    }
                    // Blend re-rank over the merged set.
...
mod tests {
    ...
    fn scanner_blocks_secrets() {
        ...
        let r = api.add(bad);
        assert!(matches!(r, Err(MnemushError::ScanBlocked(_))));
    }
...
mod action_business_logic_tests {
    ...
    /// Semantic recall: with embeddings stored, a query with zero FTS
    /// overlap (garbage tokens) still surfaces the nearest memory by
    /// cosine, because the full-corpus vector top-K joins the candidate
    /// set. Verified against a stub embedding table + fake embedder.
    ///
    /// #[ignore]: the embedder loads a local model — run `mnemush embed`
    /// once (or download all-MiniLM-L6-v2) before `cargo test -- --ignored`.
    #[test]
    #[ignore]
    fn semantic_recall_surfaces_vector_neighbors() {
        use crate::embeddings::put_embedding_tx;

        let (store, mut cfg) = store();
        cfg.embedding.enabled = true;
        let api = MemoryApi::new(&store, &cfg);
        let m1 = api.add(make_mem("alpha content", "alpha")).unwrap();
        let m2 = api.add(make_mem("beta content", "beta")).unwrap();
        ...
        // Query that has zero FTS overlap with the corpus.
        let hits = api
            .search(
                "zzzzqqqq",
                SearchOpts {
    ...
    fn auto_merge_consolidates_near_duplicate_notes() {
        ...
        let api = MemoryApi::new(&store, &cfg);
        // First note.
        let content = "mnemush uses content_hash dedup which only catches exact duplicates.             Evolving documents like SKILL.md change slightly between sessions and bypass it.";
        let id1 = api.add(make_mem(content, "evolving doc")).unwrap().id;
        // A second, near-identical note (one word changed).
        let content2 = "mnemush uses content_hash dedup which only catches exact duplicates.             Evolving documents like SKILL.md change slightly between sessions and bypass it!";
