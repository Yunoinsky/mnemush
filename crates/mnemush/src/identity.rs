...
//! `mnemush identity list-pending` to see them and `approve` / `reject`
...
/// Default identity dir: `~/.mnemush/identity/`.
...
pub fn propose_in(
    ...
    let line = serde_json::to_string(&update)
        .map_err(|e| MnemushError::Other(format!("serialize proposal: {}", e)))?;
    writeln!(file, "{}", line)?;
    Ok(update)
}
    ...
    for line in content.lines() {
        ...
        match serde_json::from_str::<PendingUpdate>(line) {
            ...
            Err(e) => {
                ...
                eprintln!("[mnemush] skipping malformed pending.jsonl line: {}", e);
            }
        }
    }
    Ok(out)
