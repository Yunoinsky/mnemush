...
//! to `~/.mneme/eval/<session>.ndjson`. Over time these accumulate:
...
fn prune_inner(dir: &Path, cfg: &EvalConfig, apply: bool) -> Result<PruneReport> {
    ...
    for entry in fs::read_dir(dir)
        .map_err(|e| MnemeError::Other(format!("read_dir {}: {}", dir.display(), e)))?
    {
        let entry = entry.map_err(|e| MnemeError::Other(e.to_string()))?;
        let path = entry.path();
