...
pub fn cached_embedder(model_id: &str) -> Result<std::sync::Arc<std::sync::Mutex<Embedder>>> {
    ...
    let mut guard = cache
        .lock()
        .map_err(|e| MnemeError::Other(format!("embedder cache mutex poisoned: {}", e)))?;
...
/// the model (cached in `~/.mneme/models/` by `fastembed`) and
...
impl Embedder {
    ...
    pub fn new(model_id: &str) -> Result<Self> {
        let model = match model_id {
            ...
            other => {
                return Err(MnemeError::Other(format!(
                    "unsupported embedding model: {} (only {} supported in this build)",
        ...
        std::fs::create_dir_all(&cache_dir)
            .map_err(|e| MnemeError::Other(format!("create model cache dir: {}", e)))?;
        let opts = InitOptions::new(model).with_cache_dir(cache_dir);
        let inner = TextEmbedding::try_new(opts)
            .map_err(|e| MnemeError::Other(format!("init embedding model: {}", e)))?;
        Ok(Self {
            inner,
    ...
    pub fn embed(&mut self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        self.inner
            .embed(texts, None)
            .map_err(|e| MnemeError::Other(format!("embed: {}", e)))
    }
